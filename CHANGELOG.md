...
- Added an automated scan for restaurants sharing an exact normalized name and address (`scripts/find-name-address-duplicates.ts`), and flagged the 28 confirmed matches it found in production as canonical/duplicate pairs (e.g. the two Paris Baguette facility IDs at the same address) — previously every `restaurant_duplicate_rules` entry was found and entered by hand, one at a time.
...
- Added compliance-tier context (e.g. "Excellent Compliance", "Marginal Compliance") to the restaurant detail panel's score hero, based on Austin's official 90/80/70 inspection score bands. The tier label sits under the score; clicking it expands a one-line explanation of what that tier means. Each tier gets its own dot color following a red → orange → yellow → green severity gradient, distinct from the plain 3-color scale used elsewhere.
- The sidebar list is now virtualized (`@tanstack/react-virtual`): only the rows within the visible scroll area exist in the DOM, instead of every matching restaurant (up to ~2,073+ in a typical view). Cuts the page's rendered DOM/button count by roughly two orders of magnitude at full result counts, with no change in scrolling, selection, or favoriting behavior.
...
- Fixed the map briefly showing six hardcoded sample restaurants (fake Austin listings) on every page load before the real data arrived. It now shows nothing on a first-ever visit, or the last successfully loaded restaurants (cached in the browser) on any later visit, until the live fetch replaces them.
- Fixed remaining search-box typing lag: the fetch debounce lived inside the data-fetching hook, tied to the raw per-keystroke value, so each character still re-ran its timer/abort-controller setup before the debounce even applied. The debounce now lives on the search value itself (`useDebouncedValue`), so a keystroke never waits on that bookkeeping — measured main-thread blocking time during a typing burst dropped by more than half.
- Fixed clearing the search box sitting on the old, smaller result set for several seconds before the map's pins caught up. The cached browse list is now restored immediately on returning to the default view, and skipped entirely (no re-fetch at all) when it's still recent — Austin's data only changes once a day, so there's no need to revalidate more often than every 15 minutes while the app is open.
- Fixed the default restaurant list (no search) taking 5.5-5.8 seconds to load: `/api/restaurants` fetched its ~4,850 rows across 5+ paginated requests awaited one at a time. Those pages are now fetched in parallel, cutting it to a consistent ~2.5-2.8 seconds, with a search or specific-restaurant request (which needs only one page) unaffected.
...
- Fixed the map's Reset and locate buttons visually overlapping Leaflet's attribution strip on mobile and tablet-portrait widths; their bottom offset wasn't large enough to clear it at that breakpoint's tighter spacing. The score legend's bottom offset was raised to match, keeping it aligned with the buttons.
