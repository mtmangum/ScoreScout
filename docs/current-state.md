...
- Regression coverage: `netlify/functions/_tests/restaurants.test.ts` mocks `supabaseRequest` with a production-scale (2,500-row), heavily name-tied synthetic dataset and simulates per-call ordering instability when the tiebreaker is missing, so this class of bug is caught by `npm test` without needing a live database connection. Verified it actually fails without the `id.asc` tiebreaker before trusting it.
...
The test suite includes scoring, URL resolution (including canonical aliases), Google Places matching, canonical inspection-history grouping, conservative facility classification, and (as of 2026-08-20) `restaurants.mts`'s Range-pagination/ordering logic against a synthetic production-scale dataset (`netlify/functions/_tests/restaurants.test.ts`).

## Fixed incidents worth knowing about

- **1.1.0 deploy failed silently to the user, production kept serving pre-fix code for ~50 minutes (fixed 2026-08-20).** After pushing the 1.1.0 data-loading rewrite, the user reported still seeing the old "999 places" symptom. `curl` with a cache-busting query param still returned the old response shape, ruling out CDN staleness — `netlify api getDeploy` showed the triggered deploy had `state: "error"`, `error_message: "Incorrect function names. Name should consist of only alphanumeric characters, hyphen & underscores"`. Cause: the new regression test was added directly at `netlify/functions/restaurants.test.ts` — Netlify auto-discovers **every** file under `[functions] directory` (`netlify.toml`) as a deployable function candidate, and a dot in the filename fails that naming check, which fails the whole build. `git push` succeeding and `netlify api listSiteDeploys` are not proof a deploy succeeded — check `state` explicitly. Fixed by moving the test to `netlify/functions/_tests/restaurants.test.ts`: Netlify's bundler already skips underscore-prefixed paths (proven by the existing `_shared/` helper directory working today), confirmed locally before repushing via `netlify functions:list`, which only lists what will actually deploy. **Lesson: any new file added under `netlify/functions/` — not just `.mts` handlers — is a function-deploy candidate unless it's under an underscore-prefixed path; run `netlify functions:list` after adding one, and verify `netlify api getDeploy`'s `state` (not just that the push/build command exited 0) before trusting that a push actually shipped.**
