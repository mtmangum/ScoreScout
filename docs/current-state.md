...
- Not changed as part of this tuning pass: the default zoom (12) still frames the *entire* Austin metro on first load, which is itself a contributing factor to a dense-feeling first impression — a smaller initial view (e.g. downtown-focused, zoom 13-14) with clustering handling the zoomed-out case is a separate, larger design decision that wasn't in scope.
