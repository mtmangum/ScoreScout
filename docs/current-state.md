...
- Default center: Austin (`30.2747, -97.7404`), zoom 15 (was 12 — see below).
...
- Default zoom raised from 12 to 15 in a follow-up change, after user feedback that the radius-100 tuning above overcorrected: at zoom 12 (whole metro), no clustering radius makes the map read as "individual pins with visible scores" — even radius 20 (tested) left 73% of restaurants inside some cluster, because showing ~1,000 real restaurants at once structurally can't look like a handful of readable pins. The product's earlier "self-evident, tied to the list" feel came from low data volume (fixtures / early partial geocoding), not from any clustering setting. Fix: narrow the *default view* instead of fighting the radius — at zoom 15, only ~180 restaurants are in view and 44% of on-screen markers are individual pins (vs. 15% at zoom 12), while still showing a real chunk of the city. Clustering still exists and still matters once a user zooms out to the full metro, where blobs are the expected trade-off. Tuning method: same real-data-driven approach as above, but simulating the actual `.map-region` viewport size/bounds at each zoom via Web Mercator math, not just total counts against a world bbox.
